...
export default function PrivateTrip() {
  ...
  async function handleSubmit(e) {
    ...
    // Validasi login
    if (!user || !token) {
      setError('Anda harus login terlebih dahulu untuk membuat permintaan trip')
      setTimeout(() => navigate('/login'), 2000)
      return
    }

    // Validasi form
    if (!formData.destinasi_id || !formData.tanggal_keberangkatan || !formData.jumlah_peserta) {
    ...
    try {
      ...
      setSuccess('Permintaan trip berhasil dikirim! Admin akan menghubungi Anda segera.')
      setFormData({
        destinasi_id: '',
        guide_id: '',
        tanggal_keberangkatan: '',
      ...
      setError(e.message || 'Gagal mengirim permintaan trip. Silakan coba lagi.')
    } finally {
      setSubmitting(false)
    }
  }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm text-slate-300">
                    Pilih Destinasi <span className="text-red-400">*</span>
                  </label>
                  <select
                    value={formData.destinasi_id}
                    onChange={(e) => setFormData({ ...formData, destinasi_id: e.target.value })}
                    required
                  ...
                  >
                    ...
                    {destinations.map((dest) => (
                      <option key={dest?.id || dest?._id} value={dest?.id || dest?._id}>
                        {dest?.nama_destinasi || dest?.nama_gunung || dest?.nama || dest?.name || 'Destinasi'}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 bg-slate-900 border border-slate-700 rounded-lg text-slate-100 focus:outline-none focus:border-cyan-400"
                  >
                    <option value="">-- Pilih Guide (Opsional) --</option>
                    {guides.map((guide) => (
                      <option key={guide?.id || guide?._id} value={guide?.id || guide?._id}>
                        {guide?.nama || guide?.name || 'Guide'} 
                        {guide?.rating ? ` (⭐ ${parseFloat(guide.rating).toFixed(1)})` : ''}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              {!user || !token ? (
                <div className="p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 text-sm">
                  <p className="mb-2">Anda harus login terlebih dahulu untuk membuat permintaan trip.</p>
                  <button
                    type="button"
                ...
                >
                  {submitting ? 'Mengirim...' : 'Buat Permintaan Trip'}
                </button>
              )}
            </form>
          </div>
