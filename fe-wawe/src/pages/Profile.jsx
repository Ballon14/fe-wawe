              ...
              <div className="flex items-center gap-6">
                ...
                {!editing && (
                  <button
                    ...
                    className="ml-4 px-6 py-3 rounded-lg border border-slate-700 bg-slate-800 text-slate-100 font-medium hover:bg-slate-700 transition-colors"
                  >
                    Ganti Password
                  </button>
                )}
            ...
            <div className="p-8 space-y-6">
              {editing ? (
                <>
                  ...
                  {/* Change password section visible when editing is false */}
                  {showChangePassword && (
                    <div className="mt-6 p-4 rounded-lg bg-slate-900 border border-slate-700">
                      <h3 className="text-lg font-semibold text-slate-100 mb-3">Ganti Password</h3>
                      <div className="space-y-3">
                        <div>
                          <label className="block text-sm text-slate-300">Password Saat Ini</label>
                          <input type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} className="w-full px-3 py-2 rounded-md bg-slate-800 border border-slate-700 text-slate-100" />
                        </div>
                        <div>
                          <label className="block text-sm text-slate-300">Password Baru</label>
                          <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full px-3 py-2 rounded-md bg-slate-800 border border-slate-700 text-slate-100" />
                        </div>
                        <div>
                          <label className="block text-sm text-slate-300">Konfirmasi Password Baru</label>
                          <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full px-3 py-2 rounded-md bg-slate-800 border border-slate-700 text-slate-100" />
                        </div>
                        <div className="flex gap-3 mt-2">
                          <button onClick={async () => {
                            setError('')
                            setSuccess('')
                            if (!currentPassword || !newPassword) { setError('Semua field harus diisi'); return }
                            ...
                            try {
                              const resp = await apiPost('/api/auth/change-password', { current_password: currentPassword, new_password: newPassword })
                              setSuccess(resp.message || 'Password berhasil diubah')
                          ...
                          }} className="px-4 py-2 rounded-lg bg-emerald-500 text-slate-900 font-semibold disabled:opacity-50" disabled={changeLoading}>{changeLoading ? 'Memproses...' : 'Simpan Password'}</button>
                          <button onClick={() => { setShowChangePassword(false); setCurrentPassword(''); setNewPassword(''); setConfirmPassword(''); setError(''); setSuccess('') }} className="px-4 py-2 rounded-lg border border-slate-700">Batal</button>
                        </div>
                      </div>
                    </div>
                  )}
